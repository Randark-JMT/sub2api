        ...
        <div v-if="windowHistory" class="card p-4">
          <div class="mb-1 flex items-center justify-between">
            ...
            <span
              ...
              :class="
                windowHistory.tracked
                  ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400'
                  : 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400'
              "
            >
              {{ t('admin.accounts.windowTracking.title') }}
            </span>
          </div>
          ...
          <!-- Opt-in hint -->
          <div
            v-if="!windowHistory.tracked"
            class="rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-700 dark:border-amber-800/40 dark:bg-amber-900/10 dark:text-amber-400"
          >
            <p class="font-medium">{{ t('admin.accounts.stats.windowHistory.optInTitle') }}</p>
            <p class="mt-1">{{ t('admin.accounts.stats.windowHistory.optInDesc') }}</p>
          </div>

          <template v-else>
            <!-- Window type tabs -->
            <div v-if="windowTabs.length > 0" class="mb-3 flex flex-wrap gap-2">
              <button
