        ...
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="mt-1 text-xs text-amber-600 dark:text-amber-400">
              {{ t('admin.accounts.windowTracking.desc') }}
            </p>
          </div>
          <button
            type="button"
            @click="windowTrackingEnabled = !windowTrackingEnabled"
            :class="[
              'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2',
              windowTrackingEnabled ? 'bg-primary-600' : 'bg-gray-200 dark:bg-dark-600'
            ]"
          >
            <span
              :class="[
                'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
                windowTrackingEnabled ? 'translate-x-5' : 'translate-x-0'
              ]"
            />
          </button>
        </div>
...
const windowTrackingEnabled = ref(false)
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  windowTrackingEnabled.value = newAccount.window_tracking_enabled === true
...
const handleSubmit = async () => {
  ...
  try {
    ...
    updatePayload.window_tracking_enabled = windowTrackingEnabled.value
    if (props.account.type === 'apikey') {
